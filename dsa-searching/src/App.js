...
function App() {
  ...
  const handleSearchLinear = (e, val) => {
    e.preventDefault();
    let res = searchFunctions.searchLinear(val);
    setState({
      ...state,
      res: res,
    });
  ...
  const handleSearchBinary = (e, val) => {
    e.preventDefault();
    let res = searchFunctions.searchBinary(val);
    console.log(res);
    setState({
      ...state,
      res: res,
  ...
  return (
    <div className="App">
      <h1>Data Structures and Algorithms - Searching</h1>
      ...
      <section className="linear-search_wrapper">
        ...
        <div>
          <form className="form">
            <label htmlFor="searchVal">Search:</label>
            <input
              id="searchVal"
              type="text"
          ...
          <div className="results_wrapper">
            <p>
              Value:
              <span className="value">{`${state.val}`}</span>
            </p>
            <p>
              Searches:
              <span className="searches">{`${state.res.count}`}</span>
            </p>
            <p>
              Found Result:
              <span className="result-val">{`${state.res.found}`}</span>
            </p>
          </div>
        </div>
      </section>
